...
void meta_oraclate(networkprogram &n)
{
	...
	//	printf("SUCCS: %d FAILS: %d SCORETOTAL: %d\n",n.succs,n.fails,n.scoretotal);
		TS += n.scoretotal;
		resetnet(n.n);
	}
}
...
void meta_meta_oraclate()
{
	for(int ROTA = 0; ROTA < 1000; ++ROTA)
	{
		...
		printf("LOWEST SCORE FOR GEN %d: %d(Succeeded %d times, Failed %d times)\n",ROTA,listOracles[listOracles.size()-1].scoretotal,listOracles[listOracles.size()-1].succs,listOracles[listOracles.size()-1].fails);
		printf("MEAN SCORE: %d\n",TS/listOracles.size());
